...
<form method="post">
    ...
    <label for="authors"><b>Authors:</b> <br>
        <input type="text" name="authors" value="{{ old('authors', implode(',', $song->authors)) }}" required>
    </label><br><hr>

    <label for="genre"><b>Genre:</b> </label><br>
    <input type="checkbox" id="genre1" name="genre[]" value="Metal" {{ (isset($song->genre) && in_array("Metal", $song->genre) || (!isset($song)) && in_array("Metal", old('genre', []))) ? "checked=\"checked\"" : "" }}>
    <label for="genre1">Metal</label><br>

    <input type="checkbox" id="genre2" name="genre[]" value="Hip hop" {{ (isset($song->genre) && in_array("Hip hop", $song->genre) || (!isset($song)) && in_array("Hip hop", old('genre', []))) ? "checked=\"checked\"" : "" }}>
    <label for="genre2">Hip hop</label><br>

    <input type="checkbox" id="genre3" name="genre[]" value="Rap" {{ (isset($song->genre) && in_array("Rap", $song->genre) || (!isset($song)) && in_array("Rap", old('genre', []))) ? "checked=\"checked\"" : "" }}>
    ...
    <?php if(isset($musician) && count($musician) == 0) echo "No musicians added yet.";
        ...
        foreach($musicians as $musician): ?>
            <input type="radio" name="musician" value="<?php echo $musician->uuid; ?>" <?php if ($musician->uuid == $song->musician) echo "checked";?>>
